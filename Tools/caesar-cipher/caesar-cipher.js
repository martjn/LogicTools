...
const keyValue = document.getElementById("src-options-key-value");
const keyTo = document.getElementById("keyTo");
...
//encode
function caesarEncode(keyValue) {
  let originalText = sourceText.value;
  let newTextArray = originalText.split("");
  let leftShift = parseInt(keyValue) % 26;
  let temp_capitals = [...capitals];
  let temp_notCapitals = [...notCapitals];
  let shiftedCapitals = temp_capitals.concat(
    temp_capitals.splice(0, leftShift)
  );
  let shiftedNotCapitals = temp_notCapitals.concat(
    temp_notCapitals.splice(0, leftShift)
  );
  for (let i = 0; i < newTextArray.length; i++) {
    if (/[A-Z]/.test(newTextArray[i])) {
      let shiftedChar = shiftedCapitals[capitals.indexOf(newTextArray[i])];
      newTextArray[i] = shiftedChar;
    } else if (/[a-z]/.test(newTextArray[i])) {
      let shiftedChar =
        shiftedNotCapitals[notCapitals.indexOf(newTextArray[i])];
      newTextArray[i] = shiftedChar;
    }
  }
  console.log(shiftedCapitals[capitals.indexOf("A")]);
  return newTextArray.join("");
}
//decode
function caesarDecode(keyValue) {
  console.log("run caesarDecode");
  let originalText = sourceText.value;
  let newTextArray = originalText.split("");
  let rightShift = parseInt(keyValue) % 26;
  let temp_capitals = [...capitals];
  let temp_notCapitals = [...notCapitals];
  let shiftedCapitals = temp_capitals
    .splice(capitals.length - rightShift)
    .concat(temp_capitals);
  let shiftedNotCapitals = temp_notCapitals
    .splice(notCapitals.length - rightShift)
    .concat(temp_notCapitals);
  for (let i = 0; i < newTextArray.length; i++) {
    if (/[A-Z]/.test(newTextArray[i])) {
      let shiftedChar = shiftedCapitals[capitals.indexOf(newTextArray[i])];
      newTextArray[i] = shiftedChar;
...
function translate() {
  let translateText;
  mode === 0
    ? (translateText = caesarDecode(keyValue.value))
    : (translateText = caesarEncode(keyValue.value));
...
function setMode() {
  if (radioButton0.checked && mode !== 0) {
    ...
    sourceText.setAttribute("placeholder", "Decode message");
    if (sourceText.value !== "") {
      translate();
    }
    ...
    currentResultText = sourceText.value;
    sourceText.removeAttribute("placeholder");
    sourceText.setAttribute("placeholder", "Encode message");
    if (sourceText.value !== "") {
      translate();
    }
  } else {
...
function removeSpaces() {
  ...
  if (
    nr === 0 &&
    sourceText.value.indexOf(" ") !== -1 &&
    sourceText.value.replaceAll(" ", "") !== currentSourceText
  ) {
  ...
  //result
    ...
    nr === 1 &&
    resultText.value.indexOf(" ") !== -1 &&
    resultText.value.replaceAll(" ", "") !== currentSourceText
  ) {
...
function lettersOnly() {
  ...
  if (nr === 0) {
    ...
    let tempSourceText = sourceText.value
      .replaceAll("0", "")
      .replaceAll("1", "")
      .replaceAll("2", "")
      .replaceAll("3", "")
  ...
  //result
    ...
    let tempResultText = sourceText.value
      .replaceAll("0", "")
      .replaceAll("1", "")
      .replaceAll("2", "")
      .replaceAll("3", "")
...
function hideOptions() {
  ...
  if (nr === 0) {
    if (srcOptionsMode) {
      ...
      document.getElementById("src-buttons-wrapper-options-1").style.display =
        "none";
      document.getElementById("src-buttons-wrapper-options-2").style.display =
        "none";
    } else {
      srcOptionsMode = true;
      document.getElementById("src-buttons-wrapper-options-1").style.display =
  ...
  //result
    ...
    if (resultOptionsMode) {
      ...
      document.getElementById(
        "result-buttons-wrapper-options-1"
      ).style.display = "none";
      document.getElementById(
        "result-buttons-wrapper-options-2"
      ).style.display = "none";
    } else {
...
const displayKey = () => {
  ...
  if (mode === 0) {
    let rightShift = parseInt(keyValue.value) % 26;
    let temp_notCapitals = [...notCapitals];
    let shiftedNotCapitals = temp_notCapitals
      .splice(notCapitals.length - rightShift)
      .concat(temp_notCapitals);
    console.log("displayKey: shiftedArray: ", shiftedNotCapitals)
    let shiftedChar = shiftedNotCapitals[notCapitals.indexOf("a")];
    keyFrom.innerHTML = "a";
    keyTo.innerHTML = shiftedChar;
  }
  // if encode
  if (mode === 1) {
    let leftShift = parseInt(keyValue.value) % 26;
    let temp_notCapitals = [...notCapitals];
    let shiftedNotCapitals = temp_notCapitals.concat(
      temp_notCapitals.splice(0, leftShift)
    );
...
keyValue.addEventListener("input", () => {
  translate();
  displayKey();
});
